...
<div class="row" style="margin: 10px;">
    ...
    <div class="panel panel-green">
        ...
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped" id="dataTables-example">
                ...
                <%
                    ...
                    string typez=Request.QueryString["type"];
                    ...
                    if (type.Count > 0)
                    {
                        foreach (var mytype in type)
                        {
                            ...
                            %>
                                ...
                                <%
                                    ...
                                    else if (mytype.Status == "Verification of Academic Resources")
                                    {
                                        ...
                                        string surrenderData = new Config().ObjNav().FnGetPASiteComponentComments(docNo);
                                        ...
                                        <tr>
                                            ...
                                            <td><a href="SubComponents.aspx?cCode=<%=arr[0] %>&&evalNo=<%=Request.QueryString["evalNo"] %>&&mystatus=<%=mytype.Status %>&&ApplicationNo=<%=mytype.Accreditation_No %>&&component=<%=arr[1] %>">
                                                ...
                                                <td>
                                                    <label class="btn btn-warning" onclick="viewResourceSubcomponents('<%=arr[0] %>','<%=arr[1] %>','<%=strength %>','<%=weakness %>','<%=rec %>','<%=aggregateScore %>','<%=maxscore %>','<%=arr[5] %>');"><i class="fa fa-pencil"></i>Edit Details</label></td>
                                                <%
                                                    }
                                                    else
                                                    {
                                                ...
                                                <td>
                                                        <label class="btn btn-info" onclick="viewResourceSubcomponents('<%=arr[0] %>','<%=arr[1] %>','<%=strength %>','<%=weakness %>','<%=rec %>','<%=aggregateScore %>','<%=maxscore %>','<%=arr[5] %>');;"><i class="fa fa-save"></i>Save Details</label></td>
                                                <%
                                                    }
                                                %>
                                                <%
                                        ...
                                        <%
                                            ...
                                            var type1 = Config.ReturnNav().LicenceApplicationHeader.Where(x => x.Application_No == ApplicationNo).ToList();
                                            if (type1.Count > 0)
                                            {
                                                foreach (var mytype in type1)
                                                {
                                                    ApplicationNo = mytype.Application_No;
                                                    if (mytype.Application_Status == "Awaiting Peer Review")
                                                    {
                                                        var projects = Config.ReturnNav().Sitecomponents.ToList();
                                                        bool applicable = false;
                                                        string strength = "";
                                                        string weakness = "";
                                                        string rec = "";
                                                        ...
                                                        string maxscore = "";
                                                        string docNo = Request.QueryString["evalNo"];
                                                        string surrenderData = new Config().ObjNav().FnGetCollaborationSiteComponentComments(docNo);
                                                        String[] info = surrenderData.Split(new string[] { "::::" }, StringSplitOptions.RemoveEmptyEntries);
                                                        if (info != null)
                                                        {
                                                            foreach (var allInfo in info)
                            ...
                            <tr>
                                <td><%=arr[0] %></td>
                                <td><%=arr[1] %></td>
                                 <td><a href="SubComponents.aspx?cCode=<%=arr[0] %>&&evalNo=<%=Request.QueryString["evalNo"] %>&&mystatus=<%=mytype.Application_Status %>&&ApplicationNo=<%=mytype.Application_No %>&&component=<%=arr[1]%>&&type=<%=typez%>">
                                    <label class="btn btn-info">Add Subcomponent Scores</label></a></td>
                                <%
                                ...
                                <td>
                                    <label class="btn btn-warning" onclick="viewResourceSubcomponents('<%=arr[0] %>','<%=arr[1] %>','<%=strength %>','<%=weakness %>','<%=rec %>','<%=aggregateScore %>','<%=maxscore %>','<%=arr[7] %>');"><i class="fa fa-pencil"></i>Edit Details</label></td>
                                <%
                                    }
                                    else
                                    {
                                ...
                                <td>
                                    <label class="btn btn-success" onclick="viewResourceSubcomponents('<%=arr[0] %>','<%=arr[1] %>','<%=strength %>','<%=weakness %>','<%=rec %>','<%=aggregateScore %>','<%=maxscore %>','<%=arr[7] %>');"><i class="fa fa-save"></i>Add Comments</label></td>
                                <%
                                    }
                                %>
                            ...
                            <%
                                ...
                                else if (mytype.Application_Status == "Awaiting Site Visit")
                                {
                                    bool applicable = false;
                                    string strength = "";
                                    string weakness = "";
                            ...
                            <tr>
                                ...
                                <td>
                                    <label class="btn btn-success" onclick="viewResourceSubcomponents('<%=arr[0] %>','<%=arr[1] %>','<%=strength %>','<%=weakness %>','<%=rec %>','<%=aggregateScore %>','<%=maxscore %>','<%=arr[7] %>');"><i class="fa fa-save"></i>Save Details</label></td>
                                <%
                                    }
                                %>
        ...
        <script>
            function viewResourceSubcomponents(lineNo, subcomponent, strength, weakness, reccomendation,aggregateComp,maxAggregateScore, entryNo) {
                ...
                document.getElementById("MainContent_aggregateComp").value = aggregateComp;
                document.getElementById("MainContent_maxAggregate").value = maxAggregateScore;
      ...
      <div class="modal-header">
       ...
       <div class="modal-body">
         ...
         <div class="form-group">
           ...
           <asp:TextBox CssClass="form-control" runat="server" ID="verdictdate" TextMode="Date" Placeholder="dd/mm/yy" />
        </div> 
      </div>
        ...
        <div class="modal-dialog" style="width: 80%">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="col-md-6 col-lg-6">
                        <strong style="color: red">Aggregate Score</strong><br />
                        <asp:TextBox runat="server" ID="aggregateComp" CssClass="form-control" ReadOnly="true" />
                    </div>
                    <div class="col-md-6 col-lg-6">
                        <strong style="color: red">Maximum Aggregate Score</strong><br />
                        <asp:TextBox runat="server" ID="maxAggregate" CssClass="form-control" ReadOnly="true" />
                    </div>
                </div>
                <div class="modal-body">
