...
var ExcelUploader = function(params){
    ...
    check_required_libs();
	//alert(params.serverColumnNames);
    let X = XLSX;
    ...
    let columnMap = ""; 
    let receiverUrl = params.receiverUrl
    ...
    function showColumnMapping(data) {
        ...
        $(tableOutputSelector).append("" +
            ...
            + "<button id='smx_finalizeBt' class='btn btn-success btn-large btn-fill'>Upload</button>"
        );
    ...
    function initUpload() {
		...
		var total = groups>100? (groups*100) :groups ;
        $(tableOutputSelector).append("<br><br>" +
            "<div id='smx_upload-block' class='form-group'>" +
            "<label>Uploading Batch 1 of " + total + "</label><br>" +
...
//                console.log("Pushing batch " + batch + " to server | start = " + start + " stop = " + stop);
    ...
    function pushDataToServer(data) {
	 ...
	 // console.log(JSON.parse(columnMap));
        let payload = $.extend({column_map: JSON.parse(serverColumnNames), data: data}, extraData);
		//alert(extraData);
	//dataType
	//console.log(extraData);
	//alert(data[1].length);
        url = receiverUrl;
        //alert(serverColumnNames);
        //alert();
        $.ajax({
            url: url,
            type: "POST",
    ...
    function updateProgress() {
		...
		var current = parseInt(batch*100); 
		var total = parseInt(groups*100);
        $("#smx_upload-block label").html("Uploaded batch " + batch + " of " + groups);
        ...
        if(batch >= groups) {
			...
			//alert("success");
			salerter("The upload has successfully completed");
            setDefaults();
